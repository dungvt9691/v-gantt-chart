...
export function getPositonOffset(time, beginTimeOfTimeLine, arg) {
  ...
  let left = 0;
  if (diffTimeByMinutes(pBeginTimeOfTimeLine, pTime) / scale > 29) {
    left = diffTimeByMinutes(pBeginTimeOfTimeLine, pTime) / scale - 29;
  } else {
    left = diffTimeByMinutes(pBeginTimeOfTimeLine, pTime) / scale;
  }
